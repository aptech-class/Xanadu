...
customElements.define('variant-radios', VariantRadios);
  ...
  class VariantColors extends HTMLElement {
  constructor() {
    super();
    this.checkedOption = Array.from(this.querySelectorAll('.grid__item input')).find((radio) => radio.checked).value;
    this.initEventListeners();
    this.updateColorMedia(this.checkedOption); 
  }
  ...
  initEventListeners() {
    this.querySelectorAll('input').forEach((input) => {
      ...
      if (label) {
        label.addEventListener('mouseenter', () => this.updateColorMedia(input.value));
        label.addEventListener('mouseleave', () => this.updateColorMedia(this.checkedOption));
      }
    });
  }

  updateColorMedia(option) {
    this.option = option;

    this.images = this.parentElement.querySelectorAll(`[data-color-image]`);
    this.firstImage = this.parentElement.querySelectorAll(`[data-color-image="${this.option}"]`)[0];
    this.secondImage = this.parentElement.querySelectorAll(`[data-color-image="${this.option}"]`)[1];

    this.images.forEach(img => {
      img.style.opacity = "0";
      img.classList.remove("first--image");
      img.classList.remove("second--image");
    });

    this.firstImage.style.opacity = "1";
    this.firstImage.classList.add("first--image");
    if (this.secondImage) {
      this.secondImage.classList.add("second--image");
    }

    this.updatePrice(this.option);
  }

  // Assuming you have other event listener for "change" event
  updateOnInputChange() {
    this.checkedOption = Array.from(this.querySelectorAll('input')).find((radio) => radio.checked).value;
    this.updateColorMedia(this.checkedOption);
  }

}

customElements.define('variant-colors', VariantColors);
